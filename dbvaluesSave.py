from dbModels.models import Agent,Party, State, Lga, Ward, Polling_unit, Announced_lga_results, Announced_pu_results
...
# script
for l in lgas:
    lga = Lga(unique_id = l[0], lga_id=l[1],lga_name=l[2],state_id = State.objects.get(pk=l[3]),lga_description=l[4],entered_by_user = l[5], user_ip_address =l[7] )
    lga.save()

# access it via: 
s = State.objects.get(pk=25)
s1=s.lga_set.all()
s1 = s1[0]
s1.lga_name
...
for w in wards:
    try:
        Lga.objects.get(pk=w[3])
        ward = Ward(unique_id=w[0],ward_id=w[1],ward_name=w[2],lga_id=Lga.objects.get(pk=w[3]), ward_description =w[4], entered_by_user =w[5])
    except:
        print(f'no lga with {w[3]} id, saving without lga')
        ward = Ward(unique_id=w[0],ward_id=w[1],ward_name=w[2], ward_description =w[4], entered_by_user =w[5])
    finally:
        ward.save()

# try:
#     Lga.objects.get(pk=0)
# except:
#     print('error occured')
...
for p in punits:
    try:
        # statement a
        unit = Polling_unit(unique_id=p[0], polling_unit_id=p[1], ward_id=Ward.objects.get(pk=p[2]),unique_ward_id=p[4],polling_unit_number = p[5], polling_unit_name=p[6], polling_unit_description=p[7], lat=p[8], long=p[9], entered_by_user =p[10], user_ip_address=p[12])
        print(f'ward{p[2]} exist  ::')

        try:
            # statement b
            # print(f'no lga with id {p[3]}')
            unit = Polling_unit(unique_id=p[0], polling_unit_id=p[1], lga_id=Lga.objects.get(pk=p[3]),unique_ward_id=p[4],polling_unit_number = p[5], polling_unit_name=p[6], polling_unit_description=p[7], lat=p[8], long=p[9], entered_by_user =p[10], user_ip_address=p[12])

            print(f'ward{p[2]} exist  ::  lga{p[3]} exist')
            unit.save()
        
        except:
            # statement a
            print(f'ward{p[2]} exist  ::  lga{p[3]} doesnt exist')
            unit = Polling_unit(unique_id=p[0], polling_unit_id=p[1], ward_id=Ward.objects.get(pk=p[2]),unique_ward_id=p[4],polling_unit_number = p[5], polling_unit_name=p[6], polling_unit_description=p[7], lat=p[8], long=p[9], entered_by_user =p[10], user_ip_address=p[12])

            unit.save()
    except:

        print(f'lga{p[3]} doesnt exist')
        try:
            # statement b
            unit = Polling_unit(unique_id=p[0], polling_unit_id=p[1], lga_id=Lga.objects.get(pk=p[3]),unique_ward_id=p[4],polling_unit_number = p[5], polling_unit_name=p[6], polling_unit_description=p[7], lat=p[8], long=p[9], entered_by_user =p[10], user_ip_address=p[12])
            print(f'ward{p[2]} exist  ::  lga{p[3]} doesnt exist')
            ...
            # statement c
            print(f'ward{p[2]} doesnt exist  ::  lga{p[3]} doesnt exist')
            unit = Polling_unit(unique_id=p[0], polling_unit_id=p[1],unique_ward_id=p[4],polling_unit_number = p[5], polling_unit_name=p[6], polling_unit_description=p[7], lat=p[8], long=p[9], entered_by_user =p[10], user_ip_address=p[12])
...
# lga_id=Lga.objects.get(pk=w[3]
# unit = Polling_unit(unique_id=p[0], polling_unit_id=p[1], ward_id=Ward.objects.get(pk=p[2]),lga_id=Lga.objects.get(pk=p[3]),unique_ward_id=p[4],polling_unit_number = p[5], polling_unit_name=p[6], polling_unit_description=p[7], lat=p[8], long=p[9], entered_by_user =p[10], user_ip_address=p[12])

# announced_lga values
alvs = [
...
# script
# method1 without table relationship with Polling_unit
# for pu in apuvs:
#     polling = Announced_pu_results(unique_id = pu[0], polling_unit_uniqueid=pu[1],party_abbreviation=pu[2],party_score=pu[3],entered_by_user =pu[4],date_entered=pu[5],user_ip_address=pu[6])
#     polling.save()
